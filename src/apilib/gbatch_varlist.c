/* gbatch_varlist.c -- API function to get variable list
...
int     gbatch_varlist(const int fd, const unsigned flags, int *np, slotno_t **slots)
{
        int             ret;
        unsigned        numvars;
        ...
        numvars = ntohl(msg.un.r_lister.nitems);
        if  (np)
                *np = (int) numvars;
        ...
        if  (numvars != 0)  {
                unsigned  nbytes = numvars * sizeof(slotno_t);
