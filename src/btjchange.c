	...
	if  (Condasschanges & OF_ASS_CHANGES)  {
		for  (jn = 0;  jn < Asscnt;  jn++)  {
			...
			if  (as->op >= BJA_SEXIT)  {
				...
				if  (as->vd.hostid)
					fprintf(dest, " %s:%s=%s",
						       look_host(as->vd.hostid), as->vd.var, as->op == BJA_SEXIT? exitcodename: signalname);
				else
					fprintf(dest, " %s=%s", as->vd.var, as->op == BJA_SEXIT? exitcodename: signalname);
			}
			else  {
				if  (as->flags)  {
				...
				if  (as->vd.hostid)
					fprintf(dest, " %s:%s%s",
						       look_host(as->vd.hostid), as->vd.var, assname[as->op-BJA_ASSIGN]);
				else
					fprintf(dest, " %s%s", as->vd.var, assname[as->op-BJA_ASSIGN]);
				dumpcon(dest, &as->con);
			}
			pch = ' ';
		}
