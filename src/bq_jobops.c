...
int  editjcvars(BtjobRef jp)
{
 ...
 redisp:
	...
	for  (ch = 0;  ch < MAXCVARS;  ch++)  {
		...
		if  (Var_seg.vlist[bcr->bjc_varind].Vent.var_id.hostid)
			mvwprintw(cw, srow+ch, BOXWID+totlen,
					 "%s:%s",
					 look_host(Var_seg.vlist[bcr->bjc_varind].Vent.var_id.hostid),
					 Var_seg.vlist[bcr->bjc_varind].Vent.var_name);
		else
			mvwaddstr(cw, srow+ch, BOXWID+totlen, Var_seg.vlist[bcr->bjc_varind].Vent.var_name);
...
int  editjsvars(BtjobRef jp)
{
 ...
 redisp:
	...
	for  (ch = 0;  ch < MAXSEVARS;  ch++)  {
		...
		if  (Var_seg.vlist[sp->bja_varind].Vent.var_id.hostid)
			mvprintw(srow+ch*2, VSNAME_P, "%s:%s",
					look_host(Var_seg.vlist[sp->bja_varind].Vent.var_id.hostid),
					Var_seg.vlist[sp->bja_varind].Vent.var_name);
		else
			mvaddstr(srow+ch*2, VSNAME_P, Var_seg.vlist[sp->bja_varind].Vent.var_name);
