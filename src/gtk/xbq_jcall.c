...
#else /* IN_XBTR */
	if  (jp->h.bj_hostid)  {
		g_string_append(labp, look_host(jp->h.bj_hostid));
		g_string_append_c(labp, ':');
	}
	g_string_append_printf(labp, "%ld", (long) jp->h.bj_job);
...
void  cb_jmac()
{
	...
	if  (jp)  {
		...
		if  (jp->h.bj_hostid)
			g_string_printf(labp, "%s %s:%ld", pr, look_host(jp->h.bj_hostid), (long) jp->h.bj_job);
		else
			g_string_printf(labp, "%s %ld", pr, (long) jp->h.bj_job);
		free(pr);
		lab = gtk_label_new(labp->str);
		g_string_free(labp, TRUE);
	}
