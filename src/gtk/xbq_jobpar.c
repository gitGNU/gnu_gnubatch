...
static char *disp_varname(BtvarRef vp, char *buf)
{
	if  (vp->var_id.hostid)  {
		sprintf(buf, "%s:%s", look_host(vp->var_id.hostid), vp->var_name);
		return  buf;
	}
	else
		return  vp->var_name;
...
void  cb_unqueue()
{
	...
	if  (cj->h.bj_hostid)
		g_string_printf(labp, "%s %s:%ld", pr, look_host(cj->h.bj_hostid), (long) cj->h.bj_job);
	else
	...
	while  (gtk_dialog_run(GTK_DIALOG(dlg)) == GTK_RESPONSE_OK)  {
		...
		if  (cj->h.bj_hostid)
			...
			sprintf(jobnobuf, "%ld", (long) cj->h.bj_job);
		ap = argbuf;
		*ap++ = udprog;
		if  (copyonly)
			*ap++ = "-n";
