...
GtkWidget *start_vardlg(BtvarRef vp, const int dlgcode, const int labelcode)
{
	...
	if  (vp->var_id.hostid)  {
		g_string_append(labp, look_host(vp->var_id.hostid));
		g_string_append_c(labp, ':');
	}
	g_string_append(labp, vp->var_name);
...
void  cb_vmac()
{
	...
	if  (vp)  {
		...
		if  (vp->var_id.hostid)
			g_string_printf(labp, "%s %s:%s", pr, look_host(vp->var_id.hostid), vp->var_name);
		else
			g_string_printf(labp, "%s %s", pr, vp->var_name);
		free(pr);
		lab = gtk_label_new(labp->str);
		g_string_free(labp, TRUE);
	}
