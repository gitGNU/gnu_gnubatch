...
static Widget  CreateVselDlg(Widget formw, const vhash_t existind, int writevars)
{
	...
	if  (existind >= 0)  {
		...
		if  (vp->var_id.hostid)  {
			char	hvname[HOSTNSIZE+BTV_NAME+2];
			sprintf(hvname, "%s:%s", look_host(vp->var_id.hostid), vp->var_name);
			XmTextSetString(workw[WORKW_JVARNAME], hvname);
		}
		else
			XmTextSetString(workw[WORKW_JVARNAME], vp->var_name);
	}
...
static void  endjunqueue(Widget w, int data)
{
	if  (data)  {
		...
		if  (cjob->h.bj_hostid)
			sprintf(jobnobuf, "%s:%ld", look_host(cjob->h.bj_hostid), (long) cjob->h.bj_job);
		else
			sprintf(jobnobuf, "%ld", (long) cjob->h.bj_job);
		ap = argbuf;
...
static void  endjfreeze(Widget w, int data)
{
	if  (data)  {
		...
		if  (!(arg0 = strrchr(udprog, '/')))
			...
			arg0++;
		execl(udprog, arg0, abuf, ismc? envprocess("$HOME"): Curr_pwd, jobnobuf, (char *) 0);
		exit(E_SETUP);
	}
	XtDestroyWidget(GetTopShell(w));
}
