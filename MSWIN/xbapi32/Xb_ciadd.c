/* Xb_ciadd.c -- add command interpeter
...
int	xb_ciadd(const int fd, const unsigned flags, const Cmdint *newci, unsigned *res)
{
	int		ret;
	...
	if  (res)
		*res = (unsigned) ntohl(msg.un.r_reader.seq);
	return  XB_OK;
}
