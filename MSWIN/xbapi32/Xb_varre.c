...
int	xb_varrename(const int fd, const unsigned flags, const slotno_t slotno, const char *newname)
{
	int	ret;
	...
	if  ((ret = xb_wmsg(fdp, &msg)))
		return  ret;
	if  ((ret = xb_write(fdp->sockfd, nbuf, sizeof(nbuf))))
		return  ret;
