...
static int job_readrest(struct api_fd *fdp, apiBtjob *result)
{
	...
	if  ((ret = xb_read(fdp->sockfd, (char *) &res.hdr.hdr, sizeof(msghdr))) != 0)
		return  ret;
	...
	if  ((ret = xb_read(fdp->sockfd, sizeof(msghdr) + (char *) &res, length)) != 0)
		return  ret;

	/* And now do all the byte-swapping */

	memset((void *) &result->h, '\0', sizeof(apiBtjobh));
	...
	result->h.bj_time		= ntohl(res.hdr.nm_time);
	result->h.bj_stime		= ntohl(res.hdr.nm_stime);
	result->h.bj_etime		= ntohl(res.hdr.nm_etime);
	...
	result->h.bj_times.tc_nexttime	= ntohl(res.hdr.nm_nexttime);
	...
	xb_mode_unpack(&result->h.bj_mode, &res.hdr.nm_mode);
	...
	if  (length > JOBSPACE)
		length = JOBSPACE;
	memcpy(result->bj_space, res.nm_space, length);
...
int	xb_jobread(const int fd, const unsigned flags, const slotno_t slotno, apiBtjob *result)
{
	int		ret;
	...
	/* The message is followed by the job details,
	   held in jobnetmsg format. We do it that way as
	   we need to hold variable refs by slot number etc */
	
	return  job_readrest(fdp, result);
}

int	xb_jobfindslot(const int fd, const unsigned flags, const jobno_t jn, const netid_t nid, slotno_t *slot)
{
	int		ret;
	struct	api_fd	*fdp = xb_look_fd(fd);
	struct	api_msg		msg;
...
int	xb_jobfind(const int fd, const unsigned flags, const jobno_t jn, const netid_t nid, slotno_t *slot, apiBtjob *jp)
{
	int		ret;
	struct	api_fd	*fdp = xb_look_fd(fd);
	struct	api_msg		msg;
